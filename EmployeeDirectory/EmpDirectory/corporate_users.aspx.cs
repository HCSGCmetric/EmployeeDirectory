...
{
    public partial class corporate_users : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            AuthorizationAttribute obj = new AuthorizationAttribute(ScreenAlias.CorporateUsers, Helper.Rights.View, true);
            if (obj.AuthorizeCore())
            {
                var permission = CommonMethods.CheckPermission(StringEnum.GetStringValue(ScreenAlias.CorporateUsers));
                if (permission != null)
                {
        ...
        [WebMethod]
        public static string GetEmployee(string empid)
        {
            string query = string.Empty;
            query += "Select ([JOB TITLE]+','+[Firstname]+','+[Lastname]+','+ case when vendor_code is null then '' else [vendor_code] end +','+AccountSegment) as employeeinfo FROM[HSGEmployeeDirectory].[dbo].[EmployeeMaster] em left outer join [HSG-SQL8].[HSG].[dbo].[apmaster_all] aa on em.[EE ID] = aa.[FTP] left outer join [HSGOPS].[dbo].csacompanymappings_vw cm on em.DEPT = cm.dept where[EE ID] = '" + empid + "'";
        ...
        {
            string query = string.Empty;
            query += "select Email as employeeinfo FROM [HSG_COUPA_Corporate_Users] where [USER] = '" + empid + "'";
